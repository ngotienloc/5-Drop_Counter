...
void lcd_put_number(I2C_LCD_HandleTypeDef *lcd, int num)
{
    char buffer[12]; // d? cho s? int 32-bit, k? c? d?u âm và null-terminator
    snprintf(buffer, sizeof(buffer), "%d", num);  // Chuy?n s? sang chu?i
    lcd_puts(lcd, buffer);  // In chu?i ra LCD
}
void lcd_clear_from(I2C_LCD_HandleTypeDef *lcd, int row, int col_start)
{
    int max_col = 20; // S?a l?i thành 16 n?u dùng LCD 16x2 ho?c 16x4
    lcd_gotoxy(lcd, col_start, row);
    for (int i = col_start; i < max_col; i++)
    {
        lcd_send_data(lcd, ' ');
    }
    lcd_gotoxy(lcd, col_start, row);  // (Tùy ch?n) Ðua con tr? v? v? trí b?t d?u
}
